...
int getIndex(string strToSearch, const string *array, int arraySize) {
  for (int i = 0; i < arraySize; i++)
    if (strcasecmp(array[i].c_str(), strToSearch.c_str()) == 0) return (i);
  return -1;
}
...
void initFixedRandomSeed(int seed) { srand(seed); }
...
void generateRandomValues(const int a, const int b, const int n,
                          vector<int> &values) {
  vector<int> index;
  values.clear();
  if (n == 0) return;
  for (unsigned int i = a; i <= b; i++) index.push_back(i);

  if (n > index.size()) {
    cerr << "Incorrect usage of generateRandomValues. Can not generate without "
            "repetitions"
         << endl;
    exit(-1);
  }
...
int generateRandom(const int minValue, const int maxValue) {
  return (int)(((double)(maxValue + 1 - minValue) * rand() /
                (RAND_MAX + 1.0))) +
         minValue;
}
...
// Nota se usa un valor truncado de pi porque en el extremo no est� definido
...
void calculateRanks(const vector<Individual *> &pop,
  ...
  do {  // Until all individuals are ranked
    ...
    for (int i = 0; i < currentPop->size(); i++) {
      ...
      for (int j = i + 1; j < currentPop->size(); j++) {
        ...
        int dominanceInformation =
            dominanceTest((*currentPop)[j], (*currentPop)[i]);
...
void markDominated(const vector<Individual *> &pop,
                   vector<int> &dominatedIndexes) {
...
void markNonDominated(const vector<Individual *> &pop,
...
void FastNondominatedSort2(vector<Individual *> *p,
                           vector<vector<Individual *> > &fronts,
                           vector<Individual *> &notClassified,
                           int &differentsInFront0, double incr) {
  ...
  vector<vector<int> > dominates(
      p->size(),
      vector<int>());  // dominates[i] --> individuals dominated by individual i
  vector<int> dominatedBy(
      p->size(),
      0);  // dominatedBy[i] --> i is dominated by such number of individuals
  vector<bool> inserted(p->size(), false);  // inserted[i] --> true: individual
                                            // i has been inserted in a front
  ...
  for (int i = 0; i < p->size(); i++) {
    ...
    if (!spaceAssigned) {
      ...
      cerr << "Internal Error: different space assigned: " << space << " "
           << spaceAssignedForId << endl;
    }
  ...
  while ((fronts[fronts.size() - 1].size() != 0) &&
         (classified < minClassified)) {
    fronts.push_back(vector<Individual *>());
    vector<Individual *> &fi = fronts[fronts.size() - 2];
    for (int i = 0; i < fi.size(); i++) {
      for (int j = 0;
           j < dominates[fi[i]->getAlgorithmData(spaceAssignedForId)].size();
           j++) {
        dominatedBy[dominates[fi[i]->getAlgorithmData(spaceAssignedForId)]
                             [j]]--;
        if (dominatedBy[dominates[fi[i]->getAlgorithmData(spaceAssignedForId)]
                                 [j]] == 0) {
          fronts[fronts.size() - 1].push_back(
              (*p)[dominates[fi[i]->getAlgorithmData(spaceAssignedForId)][j]]);
          inserted[dominates[fi[i]->getAlgorithmData(spaceAssignedForId)][j]] =
              true;
          classified++;
        }
      }
...
/**
 * Clasify p in fronts. When at least minN has been classified, the remaining
 * are not classified
 *
 * */
void FastNondominatedSort(vector<Individual *> *p,
                          vector<vector<Individual *> > &fronts,
                          vector<Individual *> &notClassified,
...
void FastNondominatedSortByFitness(vector<Individual *> *p,
  ...
  fronts.clear();
  notClassified.clear();
  // dominates[i] --> individuals dominated by individual i
  vector<vector<int> > dominates(p->size(), vector<int>());
  // dominatedBy[i] --> i is dominated by such number of individuals
  vector<int> dominatedBy(p->size(), 0);
  // inserted[i] --> true: individual i has been inserted in a front
  vector<bool> inserted(p->size(), false);
  ...
  for (int i = 0; i < p->size(); i++) {
    for (int j = i + 1; j < p->size(); j++) {
      // Ordenamos dando prioridad a los individuos con menor Fitness ID(S) para
      // MPP
      if ((*p)[i]->getFitnessValue() > (*p)[j]->getFitnessValue()) {
        dominates[i].push_back(j);
      } else if ((*p)[j]->getFitnessValue() > (*p)[i]->getFitnessValue()) {
        dominates[j].push_back(i);
      } else {
        int dominanceInfo = dominanceTest((*p)[i], (*p)[j]);
        if (dominanceInfo == FIRST_DOMINATES) {
...
void crowOrder(vector<Individual *> &pop) {
  ...
  // for (unsigned int i = 0; i < pop.size(); i++) (pop)[i]->setFitnessValue(0);
  ...
  for (unsigned int i = 0; i < (pop)[0]->getNumberOfObj(); i++) {
    ...
    // En los limities, en caso de igualdad ponemos el que mayor valor actual
    // tenga de fitness Es para tratar un poco mejor las repeticiones De todas
    // formas, el algoritmo no se comporta bien frente a las repeticiones En
    // algunos articulos se elimina de uno en uno, y no todoas a la vez, para
    // evitar estos problemas (aunque eso conllevaria mas computo)
    ...
    for (unsigned int j = 1; j < pop.size() - 1; j++) {
      if ((pop)[j]->getFitnessValue() != DBL_MAX) {
        (pop)[j]->setFitnessValue(
            (((pop)[j + 1]->getObj(i) - (pop)[j - 1]->getObj(i)) /
             (difference)) +
            (pop)[j]->getFitnessValue());
      }
    }
  }
...
// Funci�n auxiliar para ordenaci�n por objetivos
...
double generateRandomNormal(double mean, double desv) {
  double v1, v2, w, z1;
  if ((z2Generated) && (mean == lastMean) &&
      (desv == lastDesv)) { /* use value from previous call */
    z1 = z2;
    z2Generated = false;
  } else {
    do {
