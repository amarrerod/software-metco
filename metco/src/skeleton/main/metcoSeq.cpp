...
// Posicion de los parametros dentro del array
...
// Caracteres que se emplean para definir los limites de los parametros en la
// linea de comandos
const char PROBLEM = '!';
const char SCORE = '_';
...
void argumentError(char *programName) {
  cout << "Correct usage: " << programName
       << " outputPath pluginPath outputPrinterModule outputFilename "
          "algoritmo "
...
int main(int argc, char *argv[]) {
  ...
  if (argv[actualArg][0] == LOCAL_SEARCH) {
    ...
    if (actualArg < argc) {
      ...
      ls = getLocalSearch(pluginPath, localSearchModuleName, localSearchArgs,
                          true);
      if (ls == NULL) {
  ...
  if ((mo != NULL) && (!ga->supportsMultiObjectivization())) {
    cerr << "Error: selected GA does not support multiObjectivization" << endl
         << flush;
    exit(-1);
  }
