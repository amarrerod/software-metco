...
class EA : public Plugin {
 public:
  ...
  /**
   * This method must be overwritten in order to print the information of the
   *algorithm
  ...
  /**
   * Checks if the algorithm has finished yet. The stopping criteria has been
   *reached.
   **/
  bool hasFinished();
  /**
  ...
  /**
   * Set a sample Individual. Useful for copy a Individual without its
   *evaluation.
   * @param Individual
   **/
  ...
  /// @endcond
  void setScoreAlgorithm(ScoreAlgorithm *sc) {
    scoreAlgorithm = (LocalScoreAlgorithm *)sc;
  }
  ...
  /**
   * \brief Method that returns the number of maximum individuals in the
   *Pareto's Front.
  ...
  static inline string getGlobalTypeStopCrit(const int i) {
    return CRIT_STOP[i];
  }
  static inline int getTypeStoppingCriterion(const string &cr) {
    return getIndex(cr, CRIT_STOP, N_CRIT_STOP);
  }
 ...
 private:
  ...
  /**
   * \brief Method fill the population with new individuals and then, evaluate
   *them. This method could be overwritten by the sub-classes.
   **/
...
typedef enum { LOCAL_TIME, LOCAL_EVALUATIONS } local_stop_criterions_ids_t;
